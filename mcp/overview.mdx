---
title: "MCP server"
description: "Give Claude, ChatGPT, Cursor, VS Code or any MCP client the ability to publish, read and reply on your social accounts."
---
...
<CardGroup cols={2}>
  ...
  <Card title="ChatGPT" icon="sparkles" href="/mcp/clients#chatgpt">
    Developer mode → Apps & Connectors → Create → paste the URL.
  </Card>
...
Fastest path of all: **[Assistants & API keys](https://plugkit.co/dashboard/mcp)**
in the dashboard walks through one client at a time, with an install button or
the exact values to paste — and then confirms when the server has actually seen
your assistant connect.
...
<CardGroup cols={2}>
  ...
  <Card title="Analytics" icon="chart-line">
    Ranked posts, trends, follower growth and best posting times — read from the
    collected store, so one call covers every account.
  </Card>
  <Card title="Campaigns & ads" icon="bullhorn">
    Comment → DM automations, broadcasts, follow-up sequences, and Meta ad
    campaigns.
  </Card>
  <Card title="Connections" icon="plug">
    Hand the user an OAuth link to connect a new account, or disconnect one.
  </Card>
  <Card title="All 78 tools" icon="wrench" href="/mcp/tools">

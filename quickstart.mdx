...
created. Every platform has its own page under **Platforms**, and the shared
model is in [Profiles & accounts](/concepts/profiles-and-accounts). You can also
do this in two clicks from the dashboard's **Connections** page.
...
Metrics are collected in the background, so one call covers every account you
connected:

```bash
curl "https://api.plugkit.co/v1/analytics/summary" \
  -H "Authorization: Bearer sk_your_key"
```
...
<CardGroup cols={2}>
  ...
  <Card title="Skip the code" icon="robot" href="/mcp/overview">
    Add the MCP server to Claude, Cursor or VS Code and do all of the above by
    asking for it.
  </Card>
</CardGroup>
